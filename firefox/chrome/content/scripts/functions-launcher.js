						...
						if(osString.match(/x86_64/)){
							...
							if(osString.match(/i686/)){
								if(aAction === "test"){
									...
									command = command+newline+"cd \""+tempfolder.path+"\" && rm -f flashplayer64* && wget http://www.webgapps.org/downloads/flash/beta/flashplayer64 && tar xvf flashplayer64 && sudo chown root:root libflashplayer.so && sudo mv libflashplayer.so /usr/lib/mozilla/plugins/ && sudo ln -s /usr/lib/mozilla/plugins/libflashplayer.so /usr/lib/firefox-addons/plugins/libflashplayer.so && rm -f flashplayer64* && mkdir ~/.mozilla/plugins/";
							...
							if(aAction === "test"){
								...
								command = command+newline+"cd \""+tempfolder.path+"\" && rm -f flashplayer32* && wget http://www.webgapps.org/downloads/flash/beta/flashplayer32 && tar xvf flashplayer32 && sudo chown root:root libflashplayer.so && sudo mv libflashplayer.so /usr/lib/mozilla/plugins/ && sudo ln -s /usr/lib/mozilla/plugins/libflashplayer.so /usr/lib/firefox-addons/plugins/libflashplayer.so && rm -f flashplayer32*";
							}
						}
						if(opt === true){
							if(aAction !== "test"){
					...
					if(flversion === "customurl"){
						...
						if(customurl.match(/flashplayer.*\.tar\.gz/)){
							...
							if(customurl.match(/http:\/\/.*/) || customurl.match(/ftp:\/\/.*/)){
								...
								if(aAction === "test"){
									command = command+newline+"cd \""+tempfolder.path+"\" && rm -f *.tar.gz* && wget "+customurl+" && tar xvf "+tarball+" && sudo chown root:root libflashplayer.so && rm -f libflashplayer.so && rm -f *.tar.gz*";
								}else{
									command = command+newline+"cd \""+tempfolder.path+"\" && rm -f *.tar.gz* && wget "+customurl+" && tar xvf "+tarball+" && sudo chown root:root libflashplayer.so && sudo mv libflashplayer.so /usr/lib/mozilla/plugins/ && sudo ln -s /usr/lib/mozilla/plugins/libflashplayer.so /usr/lib/firefox-addons/plugins/libflashplayer.so && rm -f *.tar.gz*";
								}
								if(opt === true){
									if(aAction !== "test"){
										command = command+newline+"sudo ln -s /usr/lib/mozilla/plugins/libflashplayer.so /opt/firefox/plugins/libflashplayer.so";
							...
							if(customurl.match(/\/home\/.*/) && !customurl.match(/http:\/\/.*/) && !customurl.match(/ftp:\/\/.*/)){
								...
								if(customfile.exists() && !customfile.isDirectory()){
									...
									if(aAction === "test"){
										command = command+newline+"cd \""+tempfolder.path+"\" && rm -f *.tar.gz* && cp "+customurl+" . && tar xvf "+tarball+" && sudo chown root:root libflashplayer.so && rm -f libflashplayer.so && rm -f *.tar.gz*";
									}else{
										command = command+newline+"cd \""+tempfolder.path+"\" && rm -f *.tar.gz* && cp "+customurl+" . && tar xvf "+tarball+" && sudo chown root:root libflashplayer.so && sudo mv libflashplayer.so /usr/lib/mozilla/plugins/ && sudo ln -s /usr/lib/mozilla/plugins/libflashplayer.so /usr/lib/firefox-addons/plugins/libflashplayer.so && rm -f *.tar.gz*";
									}
									if(opt === true){
										if(aAction !== "test"){
											command = command+newline+"sudo ln -s /usr/lib/mozilla/plugins/libflashplayer.so /opt/firefox/plugins/libflashplayer.so";
